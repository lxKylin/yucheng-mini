---
agent: agent
description: M2 数据层 & 业务逻辑模块开发 —— 愈程记小程序
---
...
export function derive(reminder: Reminder): DerivedReminder {
  ...
  const progress = reminder.status === "paused" ? calcProgress(reminder.lastDate, nextDate, todayStr) : calcProgress(reminder.lastDate, nextDate, todayStr);
...
import { DEFAULT_REMIND_TIME, DEFAULT_INTERVAL, DEFAULT_BEFORE, HISTORY_MAX } from "@/constants";
...
export const useReminderStore = create<ReminderStore>((set, get) => ({
  ...
  updateReminder(id, payload) {
    set((state) => {
      const next = state.reminders.map((r) => (r.id === id ? { ...r, ...payload, updatedAt: new Date().toISOString() } : r));
      saveReminders(next);
      return { reminders: next };
    });
  },
  ...
  deleteReminder(id) {
    ...
    set((state) => {
      const next = state.reminders.map((r) => (r.id === id ? { ...r, status: "deleted" as const, updatedAt: new Date().toISOString() } : r));
      saveReminders(next);
      return { reminders: next };
    });
  },
  ...
  getById(id) {
    const r = get().reminders.find((r) => r.id === id && r.status !== "deleted");
