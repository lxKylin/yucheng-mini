...
当前应用使用 Taro 原生 tabBar，底部主导航为首页、提醒、药箱、我的四个 Tab。首页和提醒页各自渲染 `FloatingAddReminder`，点击后打开“新增提醒” BottomSheet，并复用 `UnifiedReminderComposer` 来创建药品资料、开药提醒或检查提醒。

这个页面级 FAB 已经承载了正确的产品心智：它不是单一页面的局部动作，也不是“选择开药或检查”的前置入口，而是统一新增入口。问题在于 FAB 分散在页面内部，且右下角位置会遮挡提醒卡片底部操作；药箱页再保留一个本地 `+` 时，也会和 TabBar 中央 `+` 形成重复。将主新增动作迁移到自定义 TabBar 中央后，新增可以成为全局稳定主操作，同时保持四 Tab 信息架构不变。
...
- 在 TabBar 中央提供固定、突出的全局 `+` 新增按钮。
- 中央 `+` 点击后直接打开“新增” BottomSheet，默认展示药品面板，且开药提醒开关默认关闭。
- 继续允许用户在 `UnifiedReminderComposer` 内切换到检查提醒，也允许在药品面板内按需开启开药提醒。
- 移除首页和提醒页原页面级右下角新增提醒 FAB，避免重复入口和卡片操作遮挡。
- 移除药箱页原页面级右下角新增药品 FAB，避免与 TabBar 中央 `+` 重复。
- 让中央按钮在所有主 Tab 上行为一致：始终打开统一新增 Sheet，而不是根据当前页面切换成不同弹窗。
...
选择该方案是因为用户点击 `+` 的目标是“新增”，不是切换页面。把它做成第五个 Tab 会导致选中态、返回路径和当前页面上下文变复杂，也会弱化四 Tab 信息架构。

备选方案是新增一个中间 Tab 或跳转到提醒页后再打开表单。该方案会产生额外导航动作，且用户在药箱页或我的页点击 `+` 时会被强制带离当前上下文，因此不采用。

### 复用统一新增表单，不增加类型选择弹窗

中央 `+` SHALL 直接打开“新增” BottomSheet，Sheet 内继续使用 `UnifiedReminderComposer`。默认类型为药品，开药提醒开关默认关闭；用户需要开药提醒时在药品面板内开启开关，需要检查提醒时在表单顶部切换。

选择该方案是因为现有统一新增入口已经采用“先进入新增上下文，再在表单内切换类型”的路径。额外弹出类型选择会增加一步决策；默认先保存药品资料，也符合“药品基础信息始终保存，开药提醒按需开启”的低摩擦路径。

### 使用共享新增宿主协调 TabBar 与页面
...
选择该方案是为了消除重复入口和遮挡。若保留页面 FAB，用户会同时看到 TabBar 中央按钮和右下角按钮，新增入口变得冗余。

### 药箱页不再保留本地新增药品 FAB

药箱页 SHALL 移除页面级右下角新增药品 FAB。中央 `+` 在药箱页仍打开统一“新增” Sheet，默认药品面板并默认关闭开药提醒。

选择该方案是为了避免同屏出现两个 `+` 造成入口重复。药品资料是“不要断药”链路的基础对象，作为统一新增的默认面板，比在药箱页额外保留局部 FAB 更清晰。

### 自定义 TabBar 负责安全区和遮挡控制

TabBar 样式 SHALL 适配底部安全区，并为中央按钮预留稳定尺寸。页面内容底部需要保留足够 padding，确保列表末尾、卡片操作区和加载状态不会被 TabBar 或中央按钮遮挡。

当新增 Sheet 或其他全屏/底部弹层打开时，中央按钮 SHALL 隐藏或禁用，避免重复触发和层级冲突。
...
- [Risk] 移除药箱页本地 FAB 后，用户在药箱空状态下可能不知道如何新增药品。→ Mitigation: 药箱空状态和中央按钮文案统一使用“新增”，并让药品面板成为默认入口。

## Migration Plan

1. 新增自定义 TabBar 组件和集中 Tab 配置，保持四个主 Tab 与现有图标资源一致。
2. 将 `src/app.config.ts` 的 tabBar 切换为 custom 模式，并保留四个 Tab 页面声明。
3. 新增共享新增协调层和 `GlobalReminderComposerHost`，复用 `BottomSheet` 与 `UnifiedReminderComposer`。
4. 在四个主 Tab 页面接入 Host，使中央 `+` 可在当前页面上下文打开新增 Sheet。
5. 移除首页、提醒页和药箱页原页面级 `FloatingAddReminder` 入口，调整空状态文案中对右下角 `+` 的描述。
6. 调整页面底部 padding、TabBar 安全区样式和弹层打开时中央按钮隐藏/禁用状态。
7. 回归验证四个 Tab 切换、中央新增、默认药品且开药提醒关闭、切换检查提醒、保存/取消关闭、列表底部操作无遮挡。

如果自定义 TabBar 在目标小程序环境出现不可接受的问题，可回滚 `app.config.ts` 到原生 tabBar，并恢复首页/提醒页页面级 FAB；业务数据不受影响。

## Open Questions

- 中央按钮在“我的”页打开新增后是否需要记录来源页用于埋点或后续分析；当前版本可以不做。
