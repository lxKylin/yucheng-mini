...
const echarts = exports;

let ctx;
let preprocessorRegistered = false;

function registerEChartsPreprocessor() {
  if (preprocessorRegistered || typeof echarts.registerPreprocessor !== 'function') {
    return;
  }
...
Component({
  ...
  observers: {
    'ec.option': function (option) {
      this.setChartOption(option);
    }
  },

  ready: function () {
    registerEChartsPreprocessor();
  ...
  methods: {
    createChart(canvas, width, height, dpr, callback, eventDprKey) {
      ...
      if (this.data.ec && this.data.ec.option) {
        ...
        chart.setOption(this.data.ec.option, true);
        this.chart = chart;
        return;
      }
